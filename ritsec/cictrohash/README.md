...
Thus there are 2^32 possible hashvalues. Thanks to the birthday paradox, finding a collision should not be too hard. <br>
<h5> The implementation </h5>
The hash.py file calculates the Cictro hashalue of a given input.
The findCollision function then creates a wordlist and tries to find collisions within that wordist.
...
curl -X POST http://fun.ritsec.club:8003/checkCollision --header "Content-Type: application/json" --data '{"str1": "AP", "str2": "PA"}'
 <br>
And the flag is: RITSEC{I_am_th3_gr3@t3st_h@XOR_3v@}
